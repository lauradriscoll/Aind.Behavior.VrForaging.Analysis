...
class AddExtraColumns:
    ...
    def cumulative_consecutive(self):
        ...
        for index, row in self.reward_sites.iterrows():
            ...
            self.reward_sites.loc[index, 'after_choice_cumulative_rewards'] = after_choice_cumulative_rewards
    ...
    def add_main_info(self):
        ...
        self.reward_sites['collected'] = self.reward_sites['reward_delivered'] * self.reward_sites['reward_amount']

        self.reward_sites.loc[:, 'depleted'] = np.where(self.reward_sites['reward_available'] == 0, 1, 0)
        
        self.reward_sites['next_visit_number'] = self.reward_sites['visit_number'].shift(-2)
        self.reward_sites['last_visit'] = np.where((self.reward_sites['next_visit_number'] == 0) & (self.reward_sites['has_choice'] == True), 1, 0)
        ...
        self.reward_sites['last_site'] = np.where(self.reward_sites['last_site'] == 0, 1, 0)
        
        return self.reward_sites
    
    def add_time_previous_intersite_interpatch(self):
        all_epochs = pd.concat([self.reward_sites, self.active_site.loc[self.active_site.label != 'RewardSite']])
        ...
        self.reward_sites = self.total_epochs.loc[self.total_epochs.label == 'RewardSite']
        
        return self.reward_sites, self.total_epochs
    ...
    def add_previous_patch_info(self):
        ...
        self.reward_sites['same_patch'] = np.where((self.reward_sites['next_patch'] != self.reward_sites['active_patch'])&(self.reward_sites['odor_label'] == self.reward_sites['next_odor'] ), 1, 0)
